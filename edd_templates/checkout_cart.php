...
global $post; ?>
<table id="edd_checkout_cart" <?php if ( ! edd_is_ajax_disabled() ) { echo 'class="ajaxed table table-hover"'; } ?>>
	<thead>
			...
			<?php foreach ( $cart_items as $key => $item ) : ?>
				<tr class="edd_cart_item" id="edd_cart_item_<?php echo esc_attr( $key ) . '_' . esc_attr( $item['id'] ); ?>" data-download-id="<?php echo esc_attr( $item['id'] ); ?>">
					...
					<td class="edd_cart_item_name">
						<?php
							if ( current_theme_supports( 'post-thumbnails' ) && has_post_thumbnail( $item['id'] ) ) {
								...
								?>
									<a href="<?php the_permalink($item['id']); ?>"><?php echo get_the_post_thumbnail( $item['id'], apply_filters( 'edd_checkout_image_size', array( 25,25 ) ) ); ?></a>
								<?php
								echo '</div>';
							}
							$item_title = edd_get_cart_item_name( $item );
							?>
							<a href="<?php the_permalink($item['id']); ?>" style="color: #222"><?php echo '<span class="edd_checkout_cart_item_title">' . esc_html( $item_title ) . '</span>'; ?></a>
							<?php
							/**
					...
					<td class="edd_cart_actions">
						...
						<a class="edd_cart_remove_item_btn btn btn-sm btn-danger" data-confirm-delete="Do you want to proceed?" href="<?php echo esc_url( wp_nonce_url( edd_remove_item_url( $key ), 'edd-remove-from-cart-' . $key, 'edd_remove_from_cart_nonce' ) ); ?>"><?php _e( 'Remove', 'easy-digital-downloads' ); ?></a>
					</td>
